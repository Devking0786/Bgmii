...
TOKEN = '7292289208:AAFEPwXyTs9DGjE7d-IgK6OYKhGFhKIhtiI'
...
error_channel_id = -1002172678630
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                   "*Choose an option:Choose an option:First Join Our Channel :- @DEVGLACIERVIP Plans :- 1 Day ₹50 Plan No 2 :- Week ₹300 Contact To Get Attack Plan @DEVGLACIERVIP.*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @DEVGLACIERVIP.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @DEVGLACIERVIP* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @DEVGLACIERVIP* 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO DDOS WORLD!* 🎉\n\n"
                                           ...
                                           "*🔍 Example: After* `/bgmi`, *enter:* `ip port duration`.\n\n"
